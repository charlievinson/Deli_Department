...
#define PRINT_LABEL_BUTTON 1210
#define EXIT_BUTTON 1300

#define PRODUCT_EDITTEXT 500
#define BAY_EDITTEXT 510
...
public:
    ...
    Deli_Chub(string c, string i, string w, string b, string d) {
        this->code = c;
        this->id = i;
        this->weight = w;
        this->bay = b;
...
// reload the deli chub database with an updated bay number for a specified deliChub
void reloadDeliChubDatabaseAndUpdate(Deli_Chub deliChub, vector<Deli_Chub> deliChubVector, string updatedBayNumber) {
    std::ofstream deli_chub_database_file("C:/Users/Charlie/OneDrive/Desktop/c++/Deli_Scale/Deli_Scale_Main/Deli_Scale_Main/deli_chub_database_file.txt");
    ...
    if (deli_chub_database_file.is_open())
    {
        for (int i = 0; i < deliChubVector.size(); i++) {
            if (deliChub.getId() != deliChubVector[i].getId()) {
                ...
                deli_chub_database_file << deliChub.getCode() << "," << deliChub.getWeight() << "," << updatedBayNumber << "," << deliChub.getDate() << endl;
            }
        }
        deli_chub_database_file.close();
    }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    TCHAR bayNumberEditText[50];
    string updateCode = "";
    string updateId = "";
    string updateWeight = "";
    string updateBay = "";
    string updateDate = "";
    Deli_Chub updatedDeliChub = Deli_Chub("", "", "", "", "");
    ...
    string trackingEditLabel = "Edit Tracking Information";


    TCHAR currentProductCodeSzFull[256] = _T("");
    TCHAR currentProductCodeSzTemp[256] = _T("");

    TCHAR trackingEditLabelSzFull[256] = _T("");
    TCHAR trackingEditLabelSzTemp[256] = _T("");
    ...
    for (int i = 0; i < trackingEditLabel.length(); i++) {
        _stprintf(trackingEditLabelSzTemp, _T("%c"), trackingEditLabel[i]);
        _tcscat(trackingEditLabelSzFull, trackingEditLabelSzTemp);
    }
...

    {
        ...
        SendMessage(bayNumberEditText, WM_SETFONT, WPARAM(CreateFont(36, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, L"Helvetica")), TRUE);
        ...
        HWND printLabelButton = CreateWindow(
            L"BUTTON",  // Predefined class; Unicode assumed 
            L"Print Label",      // Button text 
            WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,  // Styles 
            825,         // x position 
            650,         // y position 
            200,        // Button width
            100,        // Button height
            hWnd,     // Parent window
            (HMENU)PRINT_LABEL_BUTTON,       // No menu.
            (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE),
            NULL);      // Pointer not needed.
    ...
    case WM_COMMAND:
    {
        ...
        {
        case SAVE_BUTTON:
            //system("C:\\Users\\Charlie\\OneDrive\\Desktop\\c++\\Deli_Scale\\Deli_Scale_Tracking_Open_Products\\x64\\Debug\\Deli_Scale_Tracking_Open_Products.exe");
            GetDlgItemText(hWnd, BAY_EDITTEXT, bayNumberEditText, 50);

            updateCode = currentDeliChub.getCode();
            updateId = currentDeliChub.getId();
            updateWeight = currentDeliChub.getWeight();
            updateBay = converter.to_bytes(bayNumberEditText);
            updateDate = currentDeliChub.getDate();
            updatedDeliChub = Deli_Chub(updateCode, updateWeight, updateBay, updateDate);

            reloadDeliChubDatabaseAndUpdate(currentDeliChub, deli_chub_database, updateBay);
            MessageBox(NULL, L"Deli Chub Updated!", L"", 0);
            DestroyWindow(hWnd);
            break;
        case PRINT_LABEL_BUTTON:
